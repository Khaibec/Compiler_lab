...
void compileStatements2(void) {
  // FIX: Sửa lỗi logic - chỉ eat semicolon khi thực sự có
  while (lookAhead->tokenType == SB_SEMICOLON) {
    eat(SB_SEMICOLON);
    // Kiểm tra xem có statement tiếp theo không
    if (lookAhead->tokenType != KW_END && lookAhead->tokenType != KW_ELSE) {
      compileStatement();
    } else {
      break;  // Dừng lại nếu gặp END hoặc ELSE
    }
  }
}
...
void compileVariable(void) {
  // FIX: Bỏ dấu ; thừa ở cuối function
  eat(TK_IDENT);
