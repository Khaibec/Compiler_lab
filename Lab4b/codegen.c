...
void genVariableAddress(Object* var) {
  ...
  /* find static level difference between current scope and variable scope */
  if (var->kind == OBJ_VARIABLE) {
    Scope* vscope = VARIABLE_SCOPE(var);
    while (scope != NULL && scope != vscope) {
      level ++;
      scope = scope->outer;
    }
    genLA(level, VARIABLE_OFFSET(var));
  } else if (var->kind == OBJ_PARAMETER) {
    Scope* vscope = PARAMETER_SCOPE(var);
    while (scope != NULL && scope != vscope) {
      level ++;
      scope = scope->outer;
    }
    ...
    /* fallback: halt (should not happen in simple cases) */
    genHL();
  }
}
...
void genVariableValue(Object* var) {
  ...
  if (var->kind == OBJ_VARIABLE) {
    ...
    if (var->paramAttrs->kind == PARAM_REFERENCE) {
      /* parameter passed by reference: load indirectly */
      genLI();
    }
  } else {
    genHL();
