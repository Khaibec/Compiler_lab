...
void compileArguments(ObjectNode* paramList) {
  ...
  case SB_LPAR:
    eat(SB_LPAR);
    if (p == NULL) error(ERR_PARAMETERS_ARGUMENTS_INCONSISTENCY, lookAhead->lineNo, lookAhead->colNo);
...
Type* compileTerm(void) {
  ...
  while (lookAhead->tokenType == SB_TIMES || lookAhead->tokenType == SB_SLASH) {
    if (lookAhead->tokenType == SB_TIMES) eat(SB_TIMES);
    else eat(SB_SLASH);
    rtype = compileFactor();
    checkIntType(type);
    checkIntType(rtype);
...
void compileTerm2(void) {
  // kept for compatibility; actual checking is done in compileTerm
  switch (lookAhead->tokenType) {
  case SB_TIMES:
  case SB_SLASH:
...
Type* compileIndexes(Type* arrayType) {
  ...
  while (lookAhead->tokenType == SB_LSEL) {
    ...
    eat(SB_RSEL);
    if (curType->typeClass != TP_ARRAY)
      error(ERR_TYPE_INCONSISTENCY, currentToken->lineNo, currentToken->colNo);
    curType = curType->elementType;
  }
  return duplicateType(curType);
}
